...
const ModalCart = () => {
  ...
  const { active, closeModalCart, addedCart } = useModalCart();

  return (
    <>
      <Overlay active={active} onClick={closeModalCart} />
      <div
        className={cn(
          "fixed top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 bg-emerald-primary border border-yellow-primary rounded-md w-[450px] z-[99999] transition-all duration-300",
          !active && "invisible scale-90 opacity-0"
        )}
      >
        ...
        <div>
          <div className="flex space-x-4 border-b border-b-yellow-primary p-2.5">
            ...
            <div className="flex-1">
              <Link
                to={`/san-pham/${addedCart?.slug}`}
                className="font-semibold text-[13px] hover:text-yellow-primary block"
              >
                {addedCart?.title}
              </Link>
              <span className="text-yellow-primary font-semibold block">
                {formatVnd(addedCart?.price)}
              </span>
            </div>
          </div>
          <div className="p-2.5">
